...
def test_add_new_user_to_db(_db, new_user):
    ...
    """
    db = _db
    assert len(db.session.query(UserModel).all()) == 0
...
def test_add_new_plant_to_db(_db, new_plant):
    """
    ...
    """
    db = _db
    assert len(db.session.query(PlantModel).all()) == 0
    ...
    plant = db.session.query(PlantModel).filter_by(name="Monstera").first()
    assert plant.latin == "Monstera Adans."
    assert plant.difficulty == 5
    assert plant.user_id == 1


def test_add_new_desc_to_db(_db, new_desc):
    """
    ...
    """
    db = _db
    assert len(db.session.query(DescriptionModel).all()) == 0
    ...
    desc = db.session.query(DescriptionModel).filter_by(plant_id=1).first()
